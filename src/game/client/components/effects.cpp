...
void CEffects::Sparks(vec2 Pos, vec2 Size)
{
	for (int i = 0; i < SPRITE_SPARK2_3 - SPRITE_SPARK1_1; i++)
	{
		CParticle p;
		p.SetDefault();
		p.m_Spr = i + SPRITE_SPARK1_1;
		p.m_Pos = Pos + vec2((random_float() - 0.5f) * Size.x, (random_float() - 0.5f) * Size.y);
		p.m_Vel = vec2(0, 0);
		p.m_LifeSpan = 0.5f;
		p.m_StartSize = Size.x;
		p.m_EndSize = 0;
		p.m_Rot = random_float() * pi * 2;
		p.m_Rotspeed = pi * 2;
		p.m_Gravity = 500;
		p.m_Friction = 0.9f;
		p.m_FlowAffected = 0.0f;
		p.m_Image = IMAGE_SPARK;
		m_pClient->m_pParticles->Add(CParticles::GROUP_GENERAL, &p);
	}

	m_pClient->m_pSounds->PlayAt(CSounds::CHN_WORLD, SOUND_HOOK_LOOP, 1.0f, Pos);
}
