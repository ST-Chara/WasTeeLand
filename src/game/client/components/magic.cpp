...
void CMagic::ConKeyMagic(IConsole::IResult *pResult, void *pUserData)
{
	CMagic *pSelf = (CMagic *)pUserData;
	if(!pSelf->m_pClient->m_Snap.m_SpecInfo.m_Active && pSelf->Client()->State() != IClient::STATE_DEMOPLAYBACK)
		pSelf->m_Active = pResult->GetInteger(0) != 0;
}

void CMagic::ConMagic(IConsole::IResult *pResult, void *pUserData)
{
	((CMagic *)pUserData)->Magic(pResult->GetInteger(0));
}

void CMagic::OnConsoleInit()
{
	Console()->Register("+magic", "", CFGFLAG_CLIENT, ConKeyMagic, this, "Open magic selector");
	Console()->Register("magic", "i[magic-id]", CFGFLAG_CLIENT, ConMagic, this, "Use magic");
}
...
void CMagic::OnRelease()
{
	m_Active = false;
}

void CMagic::OnMessage(int MsgType, void *pRawMsg)
{
}

bool CMagic::OnCursorMove(float x, float y, int CursorType)
{
	if(!m_Active)
		return false;
...
void CMagic::DrawCircle(float x, float y, float r, int Segments)
{
...
void CMagic::OnRender()
{
	...
	if(!m_Active)
	{
		if(m_WasActive && m_SelectedMagic != -1)
			Magic(m_SelectedMagic);
		m_WasActive = false;
		return;
	}
	...
	if (length(m_SelectorMouse) > 110.0f)
		m_SelectedMagic = (int)(SelectedAngle / (2*pi) * NUM_EMOTICONS);
	else
		m_SelectedMagic = -1;
	...
	for (int i = 0; i < NUM_EMOTICONS; i++)
	{
		...
		bool Selected = m_SelectedMagic == i;
...
void CMagic::Magic(int Magic)
{
	CNetMsg_Cl_Magic Msg;
	Msg.m_Magic = Magic;
