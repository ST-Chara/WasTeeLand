...
void CHud::RenderHealthAndAmmo(const CNetObj_Character *pCharacter)
{
	...
	if (pCharacter->m_Weapon == WEAPON_NINJA)
	{
		...
		RenderNinjaBar(x, y + 36.f, NinjaProgress);
	}
	else
	{
	...
	float ManaProgress = clamp(pCharacter->m_Mana, 0, Max) / (float)Max;
	RenderManaBar(x, y + 48.f, ManaProgress);
...
{
	if (m_pClient->m_LocalClientID != -1 && !(m_pClient->m_Snap.m_paPlayerInfos[m_pClient->m_LocalClientID]->m_PlayerFlags & PLAYERFLAG_READY))
	{
		static CTextCursor s_Cursor(16.0f);
...
void CHud::RenderManaBar(float x, float y, float Progress)
{
	Progress = clamp(Progress, 0.0f, 1.0f);
	const float EndWidth = 6.0f;
	const float BarHeight = 12.0f;
