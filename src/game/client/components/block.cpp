...
void CBlock::OnMapLoad()
{
	ResetBlocks();
}

void CBlock::ResetBlocks()
{
	if (m_pBlocks)
		delete m_pBlocks;
...
void CBlock::SetBlock(ivec2 Pos, int Block)
{
	if (!m_pBlocks)
		ResetBlocks();
...
void CBlock::RenderBlocks(float Alpha)
{
	if (!m_Width)
		return;
	...
	*/
	...
	vec2 Center = *m_pClient->m_pCamera->GetCenter() / 32;
...
void CBlock::OnRender()
{
	...
	if (m_pClient->m_Snap.m_aCharacters[m_pClient->m_LocalClientID].m_Cur.m_Weapon == WEAPON_BLOCK)
	{
		vec2 Pos = m_pClient->m_LocalCharacterPos;
		vec2 TargetPos_1(m_pClient->m_aClients[m_pClient->m_LocalClientID].m_Predicted.m_Input.m_TargetX, m_pClient->m_aClients[m_pClient->m_LocalClientID].m_Predicted.m_Input.m_TargetY);
		vec2 TargetPos = Pos + TargetPos_1;
		ivec2 iPos = ivec2(TargetPos.x, TargetPos.y);
		if(!Collision()->GetBlock(iPos.x, iPos.y))
		{
			SetBlock(iPos, 1);
			RenderBlocks(1.f);
			SetBlock(iPos, 0);
