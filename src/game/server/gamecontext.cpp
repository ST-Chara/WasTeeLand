...
void CGameContext::CreateSparks(vec2 Pos, int Size, int Owner, int Weapon, int MaxDamage, bool SelfDamage)
{
	CNetEvent_Sparks *pEv = (CNetEvent_Sparks *)m_Events.Create(NETEVENTTYPE_SPARKS, sizeof(CNetEvent_Sparks));
	if (pEv)
	{
		pEv->m_X = (int)Pos.x;
		pEv->m_Y = (int)Pos.y;
	...
	// deal damage
	CCharacter *apEnts[MAX_CLIENTS];
	float Radius = Size * 1.f;
	float InnerRadius = Size / 1.25f;
	float MaxForce = g_pData->m_Explosion.m_MaxForce;
	int Num = m_World.FindEntities(Pos, Radius, (CEntity **)apEnts, MAX_CLIENTS, CGameWorld::ENTTYPE_CHARACTER);
	for (int i = 0; i < Num; i++)
	{
		if (!SelfDamage && apEnts[i]->GetPlayer()->GetCID() == Owner)
			continue;
