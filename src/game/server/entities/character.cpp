...
void CCharacter::FireWeapon()
{
	...
	if (m_CurrentMana >= g_pData->m_Weapons.m_aId[m_ActiveWeapon].m_ManaNeeded)
		m_CurrentMana -= g_pData->m_Weapons.m_aId[m_ActiveWeapon].m_ManaNeeded;
	else
	{
		GameServer()->CreateSound(m_Pos, SOUND_WEAPON_NOAMMO);
		return;
	...
	case WEAPON_SWORD:
	{
		vec2 ProjStartPos1 = m_Pos + Direction * GetProximityRadius() * 2.75f;
		GameServer()->CreateSound(m_Pos, SOUND_HAMMER_FIRE);

		CCharacter *apEnts[MAX_CLIENTS];
		int Hits = 0;
		int Num = GameWorld()->FindEntities(ProjStartPos1, GetProximityRadius() * 0.9f, (CEntity **)apEnts,
											MAX_CLIENTS, CGameWorld::ENTTYPE_CHARACTER);
		...
		for (int i = 0; i < Num; ++i)
		{
			...
			// set his velocity to fast upward (for now)
			if (length(pTarget->m_Pos - ProjStartPos1) > 0.0f)
				GameServer()->CreateHammerHit(pTarget->m_Pos - normalize(pTarget->m_Pos - ProjStartPos1) * GetProximityRadius() * 0.5f);
			else
				GameServer()->CreateHammerHit(ProjStartPos1);
			...
			pTarget->TakeDamage(vec2(0.f, -1.f) + normalize(Dir + vec2(0.f, -1.1f)) * 10.0f, Dir * -1, g_pData->m_Weapons.m_Sword.m_pBase->m_Damage,
								m_pPlayer->GetCID(), m_ActiveWeapon);
			Hits++;
		}
	...
	case WEAPON_SPARK:
	{
		new CProjectile(GameWorld(), WEAPON_SPARK,
						m_pPlayer->GetCID(),
						ProjStartPos,
						Direction,
...
void CCharacter::Tick()
{
	...
	// This make a slow animation of mana bar ;)
	if (m_CurrentMana > m_Mana)
		m_Mana++;
	else if (m_CurrentMana < m_Mana)
		m_Mana--;
...
{
	int Max = g_pData->m_Mana.m_Max;
	if (m_CurrentMana >= Max)
		return false;
	m_CurrentMana = clamp(m_CurrentMana + Amount, 0, Max);
	return true;
}
...
void CCharacter::Die(int Killer, int Weapon)
{
	...
	else
	{
		...
		if (GameServer()->GetPlayerChar(Killer))
			GameServer()->GetPlayerChar(Killer)->IncreaseMana(10 + m_Mana);
	}
	GameServer()->Console()->Print(IConsole::OUTPUT_LEVEL_DEBUG, "game", aBuf);
