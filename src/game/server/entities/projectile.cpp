...
CProjectile::CProjectile(CGameWorld *pGameWorld, int Type, int Owner, vec2 Pos, vec2 Dir, int Span,
						 int Damage, bool Explosive, float Force, int SoundImpact, int Weapon, bool CreateSpark)
...
vec2 CProjectile::GetPos(float Time)
{
	...
	case WEAPON_SPARK:
		Curvature = GameServer()->Tuning()->m_SparkCurvature;
		Speed = GameServer()->Tuning()->m_SparkSpeed;
	}
...
void CProjectile::Tick()
{
	...
	if (TargetChr || Collide || m_LifeSpan < 0 || GameLayerClipped(CurPos))
	{
		...
		if (m_CreateSpark)
			GameServer()->CreateSparks(CurPos, 120, m_Owner, m_Weapon, m_Damage);
	...
	{
		if(m_CreateSpark)
			GameServer()->CreateSparks(CurPos, 50, m_Owner, m_Weapon, 1, true);
	}
}
