...
CBlock::CBlock(CGameWorld *pGameWorld, int Type, vec2 Pos)
	: CEntity(pGameWorld, CGameWorld::ENTTYPE_BLOCK, vec2(int(Pos.x / 32) * 32, int(Pos.y / 32) * 32))
{
...
void CBlock::Snap(int SnappingClient)
{
	...
	CNetObj_Block *pP = static_cast<CNetObj_Block *>(Server()->SnapNewItem(NETOBJTYPE_BLOCK, GetID(), sizeof(CNetObj_Block)));
	if (!pP)
		return;
